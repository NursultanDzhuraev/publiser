...
alter table book add column county varchar;
...
alter table language add constraint unique_language unique (language);
alter table author add constraint unique_email unique (email);
truncate table language cascade ;
alter table author rename column fate_of_brith to date_of_brith;
alter type gender add value 'Male';
alter type genre add value 'ROMANS';
alter table book drop constraint book_language_id_fkey;
alter table book add constraint book_language_id_fkey foreign key (language_id) references language(id);
